...
import { equipoService, Equipo, CreateEquipoRequest } from '../services/api';
...
export default function Equipos() {
  ...
  const handleSubmit = async (values: CreateEquipoRequest) => {
    try {
      if (editingEquipo) {
        await equipoService.update(editingEquipo.id!, values);
        message.success('Equipo actualizado exitosamente');
      } else {
        await equipoService.create(values);
  ...
  const filteredEquipos = equipos.filter(equipo =>
    ...
    equipo.codigo.toLowerCase().includes(searchText.toLowerCase()) ||
  ...
  const totalValue = equipos.reduce((sum, item) => sum + (item.stock * item.costo), 0);
  const lowStockCount = equipos.filter(item => item.stock <= 5).length;
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        ...
        <Col xs={24} sm={12} md={6}>
          <Card>
            <Statistic
              title="Stock Total"
              value={equipos.reduce((sum, item) => sum + item.stock, 0)}
        ...
        >
          <Row gutter={16}>
            ...
            <Col span={12}>
              ...
              >
                <Input
                  placeholder="Ej: Tractor, Aspersora, Mochila"
                  size="large"
                />
              </Form.Item>
            </Col>
          ...
          >
            <TextArea
              placeholder="Describe las características, especificaciones y uso del equipo..."
              rows={3}
              showCount
              maxLength={300}
            />
